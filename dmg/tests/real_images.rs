...
fn hfsplus_virtual_disk_size_is_4mib() {
    ...
    assert_eq!(open("hfsplus_udro.dmg").virtual_disk_size(), 4 * 1024 * 1024);
}
...
fn hfsplus_seek_from_end() {
    ...
    let pos = r.seek(SeekFrom::Current(0)).expect("stream_position");
    assert_eq!(pos, size, "after reading last sector, position must equal virtual_disk_size");
}
...
fn hfsplus_hfs_volume_header_magic() {
    ...
    // Magic: 0x482B ("H+")
    let mut r = open("hfsplus_udro.dmg");
    r.seek(SeekFrom::Start(40 * 512 + 1024)).expect("seek to HFS+ volume header");
    let mut buf = [0u8; 2];
    r.read_exact(&mut buf).expect("read HFS+ magic");
    assert_eq!(buf, [0x48, 0x2B], "HFS+ volume header magic must be 0x482B at offset 21504");
}
...
fn compressed_dmg_sector0_matches_uncompressed() {
    ...
    comp.read_exact(&mut comp_buf).expect("read compressed sector 0");

    assert_eq!(raw_buf, comp_buf, "sector 0 must decompress to same bytes as raw UDRO image");
}
...
fn compressed_dmg_hfs_volume_header_magic() {
    ...
    let mut buf = [0u8; 2];
    r.read_exact(&mut buf).expect("read HFS+ magic");
    assert_eq!(buf, [0x48, 0x2B], "compressed DMG must decompress to valid HFS+ at offset 21504");
}
